...
FluxBasedStrainIncrement::computeQpProperties()
{
  ...
  _strain_increment[_qp] = -0.5 * (_flux_grad_tensor + _flux_grad_tensor.transpose()) - 0.5 * ((_derivative_conc[_qp] - _source[_qp]) * _Identity_tensor) - ((_Lambda_prefactor_J[_qp])*(_derivative_conc[_qp]-_source[_qp])*_Identity_tensor) + ((_Lambda_prefactor_P[_qp])*_source[_qp]*_Identity_tensor); 
  ...
  _flux_deviotoric_localC[_qp] = -0.5 * (_flux_grad_tensor + _flux_grad_tensor.transpose()) - 0.5 * ((_derivative_conc[_qp] - _source[_qp]) * _Identity_tensor) - ((_Lambda_prefactor_J[_qp])*(_derivative_conc[_qp]-_source[_qp])*_Identity_tensor) + ((_Lambda_prefactor_P[_qp])*_source[_qp]*_Identity_tensor); 
