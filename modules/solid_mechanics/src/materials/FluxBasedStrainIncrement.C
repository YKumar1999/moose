...
FluxBasedStrainIncrement::computeQpProperties()
{
  ...
  _strain_increment[_qp] = -0.5 * (_flux_grad_tensor + _flux_grad_tensor.transpose()) + 0.5 * (_flux_grad_tensor.trace() * _Identity_tensor) + ((_Lambda_prefactor[_qp])*_derivative_conc[_qp]*_Identity_tensor); 
