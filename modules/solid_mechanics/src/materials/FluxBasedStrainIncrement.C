...
FluxBasedStrainIncrement::validParams()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("flux_trace_name","Value of flux_trace");
  return params;
}
...
FluxBasedStrainIncrement::FluxBasedStrainIncrement(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _flux_trace(
        declareProperty<Real>(getParam<MaterialPropertyName>("flux_trace_name")))
...
FluxBasedStrainIncrement::computeQpProperties()
{
  ...
  _strain_increment[_qp] = -0.5 * (_flux_grad_tensor + _flux_grad_tensor.transpose()) + 0.5 * ((_flux_grad_tensor.trace()) * _Identity_tensor) - ((_Lambda_prefactor_J[_qp])*(_flux_grad_tensor.trace())*_Identity_tensor);
  _strain_increment[_qp] *= _dt;
  //Storing the tensor value
  _flux_tensor[_qp] = _flux_grad_tensor; 
  _flux_shear[_qp] = 0.5 * (_flux_grad_tensor + _flux_grad_tensor.transpose()) - 0.5 * ((_flux_grad_tensor.trace()) * _Identity_tensor);
  _flux_total[_qp] = 0.5 * (_flux_grad_tensor + _flux_grad_tensor.transpose());
  _flux_deviotoric_localC[_qp] = -0.5 * (_flux_grad_tensor + _flux_grad_tensor.transpose()) + 0.5 * ((_flux_grad_tensor.trace()) * _Identity_tensor) - ((_Lambda_prefactor_J[_qp])*(_flux_grad_tensor.trace())*_Identity_tensor);
  _flux_transpose[_qp] = 0.5 * _flux_grad_tensor.transpose();
  _flux_identity[_qp] = _Identity_tensor;
  _flux_trace[_qp] = _flux_shear[_qp].trace();
}
