...
[Materials]
  ...
  [Top]
    type = DerivativeParsedMaterial
    expression = '1e-5*mask' #'${R_sink}*(${sigma}-(mu_v/${Va}))*(1e-5*mask/${delta}*${C_sink})'
  ...
  [Right]
    type = DerivativeParsedMaterial
    expression = '-1e-5*mask1' #'${R_sink}*(${sigma}-(mu_v/${Va}))*(-1e-5*mask1/${delta}*${C_sink})'
    material_property_names = 'mask1'
...
[Executioner]
  ...
  l_max_its = 5 # SK
  nl_max_its = 5 # SK
  nl_abs_tol = 1e-5 #1e-8
  nl_rel_tol = 1e-5 #1e-08 # SK
  end_time = 1e5
  dt = 0.1
...
[Outputs]
  exodus = true
  checkpoint = true
  file_base = Testing #DiffuseGB_SquareGrain_Dimensionalize_Modified
[]
